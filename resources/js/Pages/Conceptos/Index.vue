...
const breadbrums = [
    ...
    {
        title: "Conceptos de Momvimientos",
        disabled: false,
        url: "",
        name_url: "",
    },
...
const { props } = usePage();
const { getConceptosApi, setConcepto, limpiarConcepto, deleteConcepto } =
    useConceptos();
const responseConceptos = ref([]);
...
const loadItems = async ({ page, itemsPerPage, sortBy }) => {
    ...
    setTimeOutLoadData = setTimeout(async () => {
        responseConceptos.value = await getConceptosApi(options.value);
        listConceptos.value = responseConceptos.value.data;
        totalItems.value = parseInt(responseConceptos.value.total);
        loading.value = false;
    }, 300);
};
const recargaConceptos = async () => {
    loading.value = true;
    listConceptos.value = [];
    options.value.search = search.value;
    responseConceptos.value = await getConceptosApi(options.value);
...
const editarConcepto = (item) => {
    setConcepto(item);
    accion_dialog.value = 1;
    open_dialog.value = true;
};
const eliminarConcepto = (item) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let respuesta = await deleteConcepto(item.id);
...
<template>
    <Head title="Conceptos de Movimientos"></Head>
    <v-container>
        ...
        <v-row class="mt-0">
            <v-col
                ...
                v-if="props.auth.user.permisos.includes('conceptos.create')"
            >
                <v-btn
                    color="primary"
                ...
                <v-card flat>
                    <v-card-title>
                        <v-row class="bg-primary d-flex align-center pa-3">
                            <v-col cols="12" sm="6" md="4"> Conceptos de Movimientos</v-col>
        ...
        <Formulario
            ...
            @envio-formulario="recargaConceptos"
