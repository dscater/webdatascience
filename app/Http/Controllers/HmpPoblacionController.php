...
use App\Models\HmpPoblacion;
use App\Models\Mes;
use Illuminate\Http\Request;
use Inertia\Inertia;

class HmpPoblacionController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/HmpPoblacions");
    }
        ...
        if ($filtro == 'todos') {
            ...
            $informacionArray = [
                [
                    "categories" => [],
                    "series" => [],
                    "titulo" => "Humillaciones y Menos Precios por parte de la Población",
            ...
            foreach ($array_info as $key_info => $info) {
                ...
                foreach ($distritos as $distrito) {
                    ...
                    $total = HmpPoblacion::select("hmp_poblacion.*")
                        ->join("anios", "anios.id", "=", "hmp_poblacion.anio_id")
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($array_info as $key_info => $info) {
                            ...
                            foreach ($distritos as $distrito) {
                                ...
                                $total1 = HmpPoblacion::select("hmp_poblacion.*")
                                    ->join("anios", "anios.id", "=", "hmp_poblacion.anio_id")
                                    ->where("anios.anio", $gestion1)
                                    ->where("distrito_id", $distrito->id)->sum($key_info);
                                $data1[] = (float)$total1;
                                $total2 = HmpPoblacion::select("hmp_poblacion.*")
                                    ->join("anios", "anios.id", "=", "hmp_poblacion.anio_id")
                        ...
                        foreach ($meses as $key_registro => $mes) {
                            ...
                            foreach ($array_info as $key_info => $info) {
                                ...
                                foreach ($distritos as $distrito) {
                                    ...
                                    $total = HmpPoblacion::select("hmp_poblacion.*")
                                        ->join("anios", "anios.id", "=", "hmp_poblacion.anio_id")
                                        ->where("hmp_poblacion.mes_id", $mes->id)
                                        ->where("anios.anio", $gestion)
                                        ->where("distrito_id", $distrito->id)->sum($key_info);
                                    $data[] = (float)$total;
                                }
                        ...
                        foreach ($categories as $key_registro => $trim) {
                            ...
                            foreach ($array_info as $key_info => $info) {
                                ...
                                foreach ($distritos as $distrito) {
                                    ...
                                    $total = HmpPoblacion::select("hmp_poblacion.*")
                                        ->join("anios", "anios.id", "=", "hmp_poblacion.anio_id")
                                        ->whereIn("hmp_poblacion.mes_id", $trimestres[$key_registro])
                                        ->where("anios.anio", $gestion)
                                        ->where("distrito_id", $distrito->id)->sum($key_info);
                                    $data[] = (float)$total;
                                }
                    ...
                    foreach ($anios as $key_registro => $anio) {
                        ...
                        foreach ($array_info as $key_info => $info) {
                            ...
                            foreach ($distritos as $distrito) {
                                ...
                                $total = HmpPoblacion::select("hmp_poblacion.*")
                                    ->where("hmp_poblacion.anio_id", $anio->id)
                                    ->where("distrito_id", $distrito->id)->sum($key_info);
                                $data[] = (float)$total;
                            }
                            $informacionArray[$key_registro]["series"][] = [
