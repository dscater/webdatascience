...
use App\Models\Mes;
use App\Models\NoAyuda;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NoAyudaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NoAyudas");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NoAyuda::select("no_ayuda.cantidad")
                    ->join("anios", "anios.id", "=", "no_ayuda.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NoAyuda::select("no_ayuda.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda.anio_id")
                                ->where("no_ayuda.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("no_ayuda.cantidad");

                            $total2 = NoAyuda::select("no_ayuda.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda.anio_id")
                                ->where("no_ayuda.distrito_id", $item->id)
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NoAyuda::select("no_ayuda.cantidad")
                                ...
                                ->where("no_ayuda.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda.mes_id", $trimestre1)
                                ->sum("no_ayuda.cantidad");
                            $total2 = NoAyuda::select("no_ayuda.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda.anio_id")
                                ->where("no_ayuda.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda.mes_id", $trimestre2)
                                ->sum("no_ayuda.cantidad");
                            $total3 = NoAyuda::select("no_ayuda.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda.anio_id")
                                ->where("no_ayuda.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda.mes_id", $trimestre3)
                                ->sum("no_ayuda.cantidad");
                            $total4 = NoAyuda::select("no_ayuda.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda.anio_id")
                                ->where("no_ayuda.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda.mes_id", $trimestre4)
                                ->sum("no_ayuda.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NoAyuda::select("no_ayuda.cantidad")
                                ->where("no_ayuda.distrito_id", $item->id)
                                ->where("no_ayuda.anio_id", $anio->id)
                                ->sum("no_ayuda.cantidad");
