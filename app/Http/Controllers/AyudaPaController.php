...
use App\Models\Mes;
use App\Models\AyudaPa;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class AyudaPaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/AyudaPas");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = AyudaPa::select("ayuda_pa.cantidad")
                    ->join("anios", "anios.id", "=", "ayuda_pa.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = AyudaPa::select("ayuda_pa.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_pa.anio_id")
                                ->where("ayuda_pa.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("ayuda_pa.cantidad");

                            $total2 = AyudaPa::select("ayuda_pa.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_pa.anio_id")
                                ->where("ayuda_pa.distrito_id", $item->id)
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = AyudaPa::select("ayuda_pa.cantidad")
                                ...
                                ->where("ayuda_pa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_pa.mes_id", $trimestre1)
                                ->sum("ayuda_pa.cantidad");
                            $total2 = AyudaPa::select("ayuda_pa.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_pa.anio_id")
                                ->where("ayuda_pa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_pa.mes_id", $trimestre2)
                                ->sum("ayuda_pa.cantidad");
                            $total3 = AyudaPa::select("ayuda_pa.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_pa.anio_id")
                                ->where("ayuda_pa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_pa.mes_id", $trimestre3)
                                ->sum("ayuda_pa.cantidad");
                            $total4 = AyudaPa::select("ayuda_pa.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_pa.anio_id")
                                ->where("ayuda_pa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_pa.mes_id", $trimestre4)
                                ->sum("ayuda_pa.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = AyudaPa::select("ayuda_pa.cantidad")
                                ->where("ayuda_pa.distrito_id", $item->id)
                                ->where("ayuda_pa.anio_id", $anio->id)
                                ->sum("ayuda_pa.cantidad");
