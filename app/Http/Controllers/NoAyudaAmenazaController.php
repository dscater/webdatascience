...
use App\Models\Mes;
use App\Models\NoAyudaAmenaza;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NoAyudaAmenazaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NoAyudaAmenazas");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NoAyudaAmenaza::select("no_ayuda_amenaza.cantidad")
                    ->join("anios", "anios.id", "=", "no_ayuda_amenaza.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NoAyudaAmenaza::select("no_ayuda_amenaza.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_amenaza.anio_id")
                                ->where("no_ayuda_amenaza.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("no_ayuda_amenaza.cantidad");

                            $total2 = NoAyudaAmenaza::select("no_ayuda_amenaza.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_amenaza.anio_id")
                                ->where("no_ayuda_amenaza.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = NoAyudaAmenaza::select("no_ayuda_amenaza.cantidad")
                                    ...
                                    ->where("no_ayuda_amenaza.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("no_ayuda_amenaza.mes_id", $mes->id)
                                    ->sum("no_ayuda_amenaza.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NoAyudaAmenaza::select("no_ayuda_amenaza.cantidad")
                                ...
                                ->where("no_ayuda_amenaza.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_amenaza.mes_id", $trimestre1)
                                ->sum("no_ayuda_amenaza.cantidad");
                            $total2 = NoAyudaAmenaza::select("no_ayuda_amenaza.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_amenaza.anio_id")
                                ->where("no_ayuda_amenaza.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_amenaza.mes_id", $trimestre2)
                                ->sum("no_ayuda_amenaza.cantidad");
                            $total3 = NoAyudaAmenaza::select("no_ayuda_amenaza.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_amenaza.anio_id")
                                ->where("no_ayuda_amenaza.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_amenaza.mes_id", $trimestre3)
                                ->sum("no_ayuda_amenaza.cantidad");
                            $total4 = NoAyudaAmenaza::select("no_ayuda_amenaza.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_amenaza.anio_id")
                                ->where("no_ayuda_amenaza.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_amenaza.mes_id", $trimestre4)
                                ->sum("no_ayuda_amenaza.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NoAyudaAmenaza::select("no_ayuda_amenaza.cantidad")
                                ->where("no_ayuda_amenaza.distrito_id", $item->id)
                                ->where("no_ayuda_amenaza.anio_id", $anio->id)
                                ->sum("no_ayuda_amenaza.cantidad");
