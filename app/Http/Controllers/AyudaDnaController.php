...
use App\Models\Mes;
use App\Models\AyudaDna;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class AyudaDnaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/AyudaDnas");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = AyudaDna::select("ayuda_dna.cantidad")
                    ->join("anios", "anios.id", "=", "ayuda_dna.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = AyudaDna::select("ayuda_dna.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_dna.anio_id")
                                ->where("ayuda_dna.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("ayuda_dna.cantidad");

                            $total2 = AyudaDna::select("ayuda_dna.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_dna.anio_id")
                                ->where("ayuda_dna.distrito_id", $item->id)
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = AyudaDna::select("ayuda_dna.cantidad")
                                ...
                                ->where("ayuda_dna.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_dna.mes_id", $trimestre1)
                                ->sum("ayuda_dna.cantidad");
                            $total2 = AyudaDna::select("ayuda_dna.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_dna.anio_id")
                                ->where("ayuda_dna.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_dna.mes_id", $trimestre2)
                                ->sum("ayuda_dna.cantidad");
                            $total3 = AyudaDna::select("ayuda_dna.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_dna.anio_id")
                                ->where("ayuda_dna.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_dna.mes_id", $trimestre3)
                                ->sum("ayuda_dna.cantidad");
                            $total4 = AyudaDna::select("ayuda_dna.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_dna.anio_id")
                                ->where("ayuda_dna.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_dna.mes_id", $trimestre4)
                                ->sum("ayuda_dna.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = AyudaDna::select("ayuda_dna.cantidad")
                                ->where("ayuda_dna.distrito_id", $item->id)
                                ->where("ayuda_dna.anio_id", $anio->id)
                                ->sum("ayuda_dna.cantidad");
