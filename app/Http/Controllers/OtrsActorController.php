...
use App\Models\Anio;
use App\Models\OtrsActor;
use App\Models\Distrito;
use App\Models\Mes;
use Illuminate\Http\Request;
use Inertia\Inertia;

class OtrsActorController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/OtrsActors");
    }
        ...
        if ($filtro == 'todos') {
            ...
            $informacionArray = [
                [
                    "categories" => [],
                    "series" => [],
                    "titulo" => "Obligaron a tener Relaciones Sexuales por distintos actores Principales",
            ...
            foreach ($array_info as $key_info => $info) {
                ...
                foreach ($distritos as $distrito) {
                    ...
                    $total = OtrsActor::select("otrs_actores.*")
                        ->join("anios", "anios.id", "=", "otrs_actores.anio_id")
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($array_info as $key_info => $info) {
                            ...
                            foreach ($distritos as $distrito) {
                                ...
                                $total1 = OtrsActor::select("otrs_actores.*")
                                    ->join("anios", "anios.id", "=", "otrs_actores.anio_id")
                                    ->where("anios.anio", $gestion1)
                                    ->where("distrito_id", $distrito->id)->sum($key_info);
                                $data1[] = (float)$total1;
                                $total2 = OtrsActor::select("otrs_actores.*")
                                    ->join("anios", "anios.id", "=", "otrs_actores.anio_id")
                        ...
                        foreach ($meses as $key_registro => $mes) {
                            ...
                            foreach ($array_info as $key_info => $info) {
                                ...
                                foreach ($distritos as $distrito) {
                                    ...
                                    $total = OtrsActor::select("otrs_actores.*")
                                        ->join("anios", "anios.id", "=", "otrs_actores.anio_id")
                                        ->where("otrs_actores.mes_id", $mes->id)
                                        ->where("anios.anio", $gestion)
                                        ->where("distrito_id", $distrito->id)->sum($key_info);
                                    $data[] = (float)$total;
                                }
                        ...
                        foreach ($categories as $key_registro => $trim) {
                            ...
                            foreach ($array_info as $key_info => $info) {
                                ...
                                foreach ($distritos as $distrito) {
                                    ...
                                    $total = OtrsActor::select("otrs_actores.*")
                                        ->join("anios", "anios.id", "=", "otrs_actores.anio_id")
                                        ->whereIn("otrs_actores.mes_id", $trimestres[$key_registro])
                                        ->where("anios.anio", $gestion)
                                        ->where("distrito_id", $distrito->id)->sum($key_info);
                                    $data[] = (float)$total;
                                }
                    ...
                    foreach ($anios as $key_registro => $anio) {
                        ...
                        foreach ($array_info as $key_info => $info) {
                            ...
                            foreach ($distritos as $distrito) {
                                ...
                                $total = OtrsActor::select("otrs_actores.*")
                                    ->where("otrs_actores.anio_id", $anio->id)
                                    ->where("distrito_id", $distrito->id)->sum($key_info);
                                $data[] = (float)$total;
                            }
                            $informacionArray[$key_registro]["series"][] = [
