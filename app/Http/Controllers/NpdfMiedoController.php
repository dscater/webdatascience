...
use App\Models\Mes;
use App\Models\NpdfMiedo;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NpdfMiedoController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NpdfMiedos");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NpdfMiedo::select("npdf_miedo.cantidad")
                    ->join("anios", "anios.id", "=", "npdf_miedo.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NpdfMiedo::select("npdf_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_miedo.anio_id")
                                ->where("npdf_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("npdf_miedo.cantidad");

                            $total2 = NpdfMiedo::select("npdf_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_miedo.anio_id")
                                ->where("npdf_miedo.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = NpdfMiedo::select("npdf_miedo.cantidad")
                                    ...
                                    ->where("npdf_miedo.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("npdf_miedo.mes_id", $mes->id)
                                    ->sum("npdf_miedo.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NpdfMiedo::select("npdf_miedo.cantidad")
                                ...
                                ->where("npdf_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_miedo.mes_id", $trimestre1)
                                ->sum("npdf_miedo.cantidad");
                            $total2 = NpdfMiedo::select("npdf_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_miedo.anio_id")
                                ->where("npdf_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_miedo.mes_id", $trimestre2)
                                ->sum("npdf_miedo.cantidad");
                            $total3 = NpdfMiedo::select("npdf_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_miedo.anio_id")
                                ->where("npdf_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_miedo.mes_id", $trimestre3)
                                ->sum("npdf_miedo.cantidad");
                            $total4 = NpdfMiedo::select("npdf_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_miedo.anio_id")
                                ->where("npdf_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_miedo.mes_id", $trimestre4)
                                ->sum("npdf_miedo.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NpdfMiedo::select("npdf_miedo.cantidad")
                                ->where("npdf_miedo.distrito_id", $item->id)
                                ->where("npdf_miedo.anio_id", $anio->id)
                                ->sum("npdf_miedo.cantidad");
