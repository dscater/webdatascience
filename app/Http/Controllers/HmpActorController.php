...
use App\Models\HmpActor;
use App\Models\Mes;
use Illuminate\Http\Request;
use Inertia\Inertia;

class HmpActorController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/HmpActors");
    }
        ...
        if ($filtro == 'todos') {
            ...
            $informacionArray = [
                [
                    "categories" => [],
                    "series" => [],
                    "titulo" => "Humillaciones y Menos Precios por actores Principales",
            ...
            foreach ($array_info as $key_info => $info) {
                ...
                foreach ($distritos as $distrito) {
                    ...
                    $total = HmpActor::select("hmp_actores.*")
                        ->join("anios", "anios.id", "=", "hmp_actores.anio_id")
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($array_info as $key_info => $info) {
                            ...
                            foreach ($distritos as $distrito) {
                                ...
                                $total1 = HmpActor::select("hmp_actores.*")
                                    ->join("anios", "anios.id", "=", "hmp_actores.anio_id")
                                    ->where("anios.anio", $gestion1)
                                    ->where("distrito_id", $distrito->id)->sum($key_info);
                                $data1[] = (float)$total1;
                                $total2 = HmpActor::select("hmp_actores.*")
                                    ->join("anios", "anios.id", "=", "hmp_actores.anio_id")
                        ...
                        foreach ($meses as $key_registro => $mes) {
                            ...
                            foreach ($array_info as $key_info => $info) {
                                ...
                                foreach ($distritos as $distrito) {
                                    ...
                                    $total = HmpActor::select("hmp_actores.*")
                                        ->join("anios", "anios.id", "=", "hmp_actores.anio_id")
                                        ->where("hmp_actores.mes_id", $mes->id)
                                        ->where("anios.anio", $gestion)
                                        ->where("distrito_id", $distrito->id)->sum($key_info);
                                    $data[] = (float)$total;
                                }
                        ...
                        foreach ($categories as $key_registro => $trim) {
                            ...
                            foreach ($array_info as $key_info => $info) {
                                ...
                                foreach ($distritos as $distrito) {
                                    ...
                                    $total = HmpActor::select("hmp_actores.*")
                                        ->join("anios", "anios.id", "=", "hmp_actores.anio_id")
                                        ->whereIn("hmp_actores.mes_id", $trimestres[$key_registro])
                                        ->where("anios.anio", $gestion)
                                        ->where("distrito_id", $distrito->id)->sum($key_info);
                                    $data[] = (float)$total;
                                }
                    ...
                    foreach ($anios as $key_registro => $anio) {
                        ...
                        foreach ($array_info as $key_info => $info) {
                            ...
                            foreach ($distritos as $distrito) {
                                ...
                                $total = HmpActor::select("hmp_actores.*")
                                    ->where("hmp_actores.anio_id", $anio->id)
                                    ->where("distrito_id", $distrito->id)->sum($key_info);
                                $data[] = (float)$total;
                            }
                            $informacionArray[$key_registro]["series"][] = [
