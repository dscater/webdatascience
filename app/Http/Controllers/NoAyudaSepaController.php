...
use App\Models\Mes;
use App\Models\NoAyudaSepa;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NoAyudaSepaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NoAyudaSepas");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NoAyudaSepa::select("no_ayuda_sepa.cantidad")
                    ->join("anios", "anios.id", "=", "no_ayuda_sepa.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NoAyudaSepa::select("no_ayuda_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_sepa.anio_id")
                                ->where("no_ayuda_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("no_ayuda_sepa.cantidad");

                            $total2 = NoAyudaSepa::select("no_ayuda_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_sepa.anio_id")
                                ->where("no_ayuda_sepa.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = NoAyudaSepa::select("no_ayuda_sepa.cantidad")
                                    ...
                                    ->where("no_ayuda_sepa.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("no_ayuda_sepa.mes_id", $mes->id)
                                    ->sum("no_ayuda_sepa.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NoAyudaSepa::select("no_ayuda_sepa.cantidad")
                                ...
                                ->where("no_ayuda_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_sepa.mes_id", $trimestre1)
                                ->sum("no_ayuda_sepa.cantidad");
                            $total2 = NoAyudaSepa::select("no_ayuda_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_sepa.anio_id")
                                ->where("no_ayuda_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_sepa.mes_id", $trimestre2)
                                ->sum("no_ayuda_sepa.cantidad");
                            $total3 = NoAyudaSepa::select("no_ayuda_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_sepa.anio_id")
                                ->where("no_ayuda_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_sepa.mes_id", $trimestre3)
                                ->sum("no_ayuda_sepa.cantidad");
                            $total4 = NoAyudaSepa::select("no_ayuda_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_sepa.anio_id")
                                ->where("no_ayuda_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_sepa.mes_id", $trimestre4)
                                ->sum("no_ayuda_sepa.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NoAyudaSepa::select("no_ayuda_sepa.cantidad")
                                ->where("no_ayuda_sepa.distrito_id", $item->id)
                                ->where("no_ayuda_sepa.anio_id", $anio->id)
                                ->sum("no_ayuda_sepa.cantidad");
