...
use App\Models\Mes;
use App\Models\AyudaSlim;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class AyudaSlimController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/AyudaSlims");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = AyudaSlim::select("ayuda_slim.cantidad")
                    ->join("anios", "anios.id", "=", "ayuda_slim.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = AyudaSlim::select("ayuda_slim.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_slim.anio_id")
                                ->where("ayuda_slim.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("ayuda_slim.cantidad");

                            $total2 = AyudaSlim::select("ayuda_slim.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_slim.anio_id")
                                ->where("ayuda_slim.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = AyudaSlim::select("ayuda_slim.cantidad")
                                    ...
                                    ->where("ayuda_slim.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("ayuda_slim.mes_id", $mes->id)
                                    ->sum("ayuda_slim.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = AyudaSlim::select("ayuda_slim.cantidad")
                                ...
                                ->where("ayuda_slim.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_slim.mes_id", $trimestre1)
                                ->sum("ayuda_slim.cantidad");
                            $total2 = AyudaSlim::select("ayuda_slim.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_slim.anio_id")
                                ->where("ayuda_slim.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_slim.mes_id", $trimestre2)
                                ->sum("ayuda_slim.cantidad");
                            $total3 = AyudaSlim::select("ayuda_slim.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_slim.anio_id")
                                ->where("ayuda_slim.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_slim.mes_id", $trimestre3)
                                ->sum("ayuda_slim.cantidad");
                            $total4 = AyudaSlim::select("ayuda_slim.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_slim.anio_id")
                                ->where("ayuda_slim.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_slim.mes_id", $trimestre4)
                                ->sum("ayuda_slim.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = AyudaSlim::select("ayuda_slim.cantidad")
                                ->where("ayuda_slim.distrito_id", $item->id)
                                ->where("ayuda_slim.anio_id", $anio->id)
                                ->sum("ayuda_slim.cantidad");
