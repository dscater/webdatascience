...
use App\Models\Mes;
use App\Models\NoAyudaNoSabia;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NoAyudaNoSabiaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NoAyudaNoSabias");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NoAyudaNoSabia::select("no_ayuda_nosabia.cantidad")
                    ->join("anios", "anios.id", "=", "no_ayuda_nosabia.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NoAyudaNoSabia::select("no_ayuda_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_nosabia.anio_id")
                                ->where("no_ayuda_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("no_ayuda_nosabia.cantidad");

                            $total2 = NoAyudaNoSabia::select("no_ayuda_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_nosabia.anio_id")
                                ->where("no_ayuda_nosabia.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = NoAyudaNoSabia::select("no_ayuda_nosabia.cantidad")
                                    ...
                                    ->where("no_ayuda_nosabia.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("no_ayuda_nosabia.mes_id", $mes->id)
                                    ->sum("no_ayuda_nosabia.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NoAyudaNoSabia::select("no_ayuda_nosabia.cantidad")
                                ...
                                ->where("no_ayuda_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_nosabia.mes_id", $trimestre1)
                                ->sum("no_ayuda_nosabia.cantidad");
                            $total2 = NoAyudaNoSabia::select("no_ayuda_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_nosabia.anio_id")
                                ->where("no_ayuda_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_nosabia.mes_id", $trimestre2)
                                ->sum("no_ayuda_nosabia.cantidad");
                            $total3 = NoAyudaNoSabia::select("no_ayuda_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_nosabia.anio_id")
                                ->where("no_ayuda_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_nosabia.mes_id", $trimestre3)
                                ->sum("no_ayuda_nosabia.cantidad");
                            $total4 = NoAyudaNoSabia::select("no_ayuda_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_nosabia.anio_id")
                                ->where("no_ayuda_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_nosabia.mes_id", $trimestre4)
                                ->sum("no_ayuda_nosabia.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NoAyudaNoSabia::select("no_ayuda_nosabia.cantidad")
                                ->where("no_ayuda_nosabia.distrito_id", $item->id)
                                ->where("no_ayuda_nosabia.anio_id", $anio->id)
                                ->sum("no_ayuda_nosabia.cantidad");
