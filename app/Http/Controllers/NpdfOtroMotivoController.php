...
use App\Models\Mes;
use App\Models\NpdfOtroMotivo;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NpdfOtroMotivoController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NpdfOtroMotivos");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NpdfOtroMotivo::select("npdf_otromotivo.cantidad")
                    ->join("anios", "anios.id", "=", "npdf_otromotivo.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NpdfOtroMotivo::select("npdf_otromotivo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_otromotivo.anio_id")
                                ->where("npdf_otromotivo.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("npdf_otromotivo.cantidad");

                            $total2 = NpdfOtroMotivo::select("npdf_otromotivo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_otromotivo.anio_id")
                                ->where("npdf_otromotivo.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = NpdfOtroMotivo::select("npdf_otromotivo.cantidad")
                                    ...
                                    ->where("npdf_otromotivo.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("npdf_otromotivo.mes_id", $mes->id)
                                    ->sum("npdf_otromotivo.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NpdfOtroMotivo::select("npdf_otromotivo.cantidad")
                                ...
                                ->where("npdf_otromotivo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_otromotivo.mes_id", $trimestre1)
                                ->sum("npdf_otromotivo.cantidad");
                            $total2 = NpdfOtroMotivo::select("npdf_otromotivo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_otromotivo.anio_id")
                                ->where("npdf_otromotivo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_otromotivo.mes_id", $trimestre2)
                                ->sum("npdf_otromotivo.cantidad");
                            $total3 = NpdfOtroMotivo::select("npdf_otromotivo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_otromotivo.anio_id")
                                ->where("npdf_otromotivo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_otromotivo.mes_id", $trimestre3)
                                ->sum("npdf_otromotivo.cantidad");
                            $total4 = NpdfOtroMotivo::select("npdf_otromotivo.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_otromotivo.anio_id")
                                ->where("npdf_otromotivo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_otromotivo.mes_id", $trimestre4)
                                ->sum("npdf_otromotivo.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NpdfOtroMotivo::select("npdf_otromotivo.cantidad")
                                ->where("npdf_otromotivo.distrito_id", $item->id)
                                ->where("npdf_otromotivo.anio_id", $anio->id)
                                ->sum("npdf_otromotivo.cantidad");
