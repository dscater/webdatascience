...
use App\Models\Mes;
use App\Models\NoAyudaMiedo;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NoAyudaMiedoController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NoAyudaMiedos");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NoAyudaMiedo::select("no_ayuda_miedo.cantidad")
                    ->join("anios", "anios.id", "=", "no_ayuda_miedo.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NoAyudaMiedo::select("no_ayuda_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_miedo.anio_id")
                                ->where("no_ayuda_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("no_ayuda_miedo.cantidad");

                            $total2 = NoAyudaMiedo::select("no_ayuda_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_miedo.anio_id")
                                ->where("no_ayuda_miedo.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = NoAyudaMiedo::select("no_ayuda_miedo.cantidad")
                                    ...
                                    ->where("no_ayuda_miedo.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("no_ayuda_miedo.mes_id", $mes->id)
                                    ->sum("no_ayuda_miedo.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NoAyudaMiedo::select("no_ayuda_miedo.cantidad")
                                ...
                                ->where("no_ayuda_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_miedo.mes_id", $trimestre1)
                                ->sum("no_ayuda_miedo.cantidad");
                            $total2 = NoAyudaMiedo::select("no_ayuda_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_miedo.anio_id")
                                ->where("no_ayuda_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_miedo.mes_id", $trimestre2)
                                ->sum("no_ayuda_miedo.cantidad");
                            $total3 = NoAyudaMiedo::select("no_ayuda_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_miedo.anio_id")
                                ->where("no_ayuda_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_miedo.mes_id", $trimestre3)
                                ->sum("no_ayuda_miedo.cantidad");
                            $total4 = NoAyudaMiedo::select("no_ayuda_miedo.cantidad")
                                ->join("anios", "anios.id", "=", "no_ayuda_miedo.anio_id")
                                ->where("no_ayuda_miedo.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("no_ayuda_miedo.mes_id", $trimestre4)
                                ->sum("no_ayuda_miedo.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NoAyudaMiedo::select("no_ayuda_miedo.cantidad")
                                ->where("no_ayuda_miedo.distrito_id", $item->id)
                                ->where("no_ayuda_miedo.anio_id", $anio->id)
                                ->sum("no_ayuda_miedo.cantidad");
