...
use App\Models\Mes;
use App\Models\NpdfSepa;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NpdfSepaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NpdfSepas");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NpdfSepa::select("npdf_sepa.cantidad")
                    ->join("anios", "anios.id", "=", "npdf_sepa.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NpdfSepa::select("npdf_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_sepa.anio_id")
                                ->where("npdf_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("npdf_sepa.cantidad");

                            $total2 = NpdfSepa::select("npdf_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_sepa.anio_id")
                                ->where("npdf_sepa.distrito_id", $item->id)
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NpdfSepa::select("npdf_sepa.cantidad")
                                ...
                                ->where("npdf_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_sepa.mes_id", $trimestre1)
                                ->sum("npdf_sepa.cantidad");
                            $total2 = NpdfSepa::select("npdf_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_sepa.anio_id")
                                ->where("npdf_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_sepa.mes_id", $trimestre2)
                                ->sum("npdf_sepa.cantidad");
                            $total3 = NpdfSepa::select("npdf_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_sepa.anio_id")
                                ->where("npdf_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_sepa.mes_id", $trimestre3)
                                ->sum("npdf_sepa.cantidad");
                            $total4 = NpdfSepa::select("npdf_sepa.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_sepa.anio_id")
                                ->where("npdf_sepa.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_sepa.mes_id", $trimestre4)
                                ->sum("npdf_sepa.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NpdfSepa::select("npdf_sepa.cantidad")
                                ->where("npdf_sepa.distrito_id", $item->id)
                                ->where("npdf_sepa.anio_id", $anio->id)
                                ->sum("npdf_sepa.cantidad");
