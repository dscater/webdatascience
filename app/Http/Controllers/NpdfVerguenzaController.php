...
use App\Models\Mes;
use App\Models\NpdfVerguenza;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NpdfVerguenzaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NpdfVerguenzas");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NpdfVerguenza::select("npdf_verguenza.cantidad")
                    ->join("anios", "anios.id", "=", "npdf_verguenza.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NpdfVerguenza::select("npdf_verguenza.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_verguenza.anio_id")
                                ->where("npdf_verguenza.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("npdf_verguenza.cantidad");

                            $total2 = NpdfVerguenza::select("npdf_verguenza.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_verguenza.anio_id")
                                ->where("npdf_verguenza.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = NpdfVerguenza::select("npdf_verguenza.cantidad")
                                    ...
                                    ->where("npdf_verguenza.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("npdf_verguenza.mes_id", $mes->id)
                                    ->sum("npdf_verguenza.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NpdfVerguenza::select("npdf_verguenza.cantidad")
                                ...
                                ->where("npdf_verguenza.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_verguenza.mes_id", $trimestre1)
                                ->sum("npdf_verguenza.cantidad");
                            $total2 = NpdfVerguenza::select("npdf_verguenza.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_verguenza.anio_id")
                                ->where("npdf_verguenza.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_verguenza.mes_id", $trimestre2)
                                ->sum("npdf_verguenza.cantidad");
                            $total3 = NpdfVerguenza::select("npdf_verguenza.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_verguenza.anio_id")
                                ->where("npdf_verguenza.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_verguenza.mes_id", $trimestre3)
                                ->sum("npdf_verguenza.cantidad");
                            $total4 = NpdfVerguenza::select("npdf_verguenza.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_verguenza.anio_id")
                                ->where("npdf_verguenza.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_verguenza.mes_id", $trimestre4)
                                ->sum("npdf_verguenza.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NpdfVerguenza::select("npdf_verguenza.cantidad")
                                ->where("npdf_verguenza.distrito_id", $item->id)
                                ->where("npdf_verguenza.anio_id", $anio->id)
                                ->sum("npdf_verguenza.cantidad");
