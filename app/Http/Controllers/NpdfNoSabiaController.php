...
use App\Models\Mes;
use App\Models\NpdfNoSabia;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NpdfNoSabiaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NpdfNoSabias");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NpdfNoSabia::select("npdf_nosabia.cantidad")
                    ->join("anios", "anios.id", "=", "npdf_nosabia.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NpdfNoSabia::select("npdf_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_nosabia.anio_id")
                                ->where("npdf_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("npdf_nosabia.cantidad");

                            $total2 = NpdfNoSabia::select("npdf_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_nosabia.anio_id")
                                ->where("npdf_nosabia.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = NpdfNoSabia::select("npdf_nosabia.cantidad")
                                    ...
                                    ->where("npdf_nosabia.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("npdf_nosabia.mes_id", $mes->id)
                                    ->sum("npdf_nosabia.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NpdfNoSabia::select("npdf_nosabia.cantidad")
                                ...
                                ->where("npdf_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_nosabia.mes_id", $trimestre1)
                                ->sum("npdf_nosabia.cantidad");
                            $total2 = NpdfNoSabia::select("npdf_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_nosabia.anio_id")
                                ->where("npdf_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_nosabia.mes_id", $trimestre2)
                                ->sum("npdf_nosabia.cantidad");
                            $total3 = NpdfNoSabia::select("npdf_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_nosabia.anio_id")
                                ->where("npdf_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_nosabia.mes_id", $trimestre3)
                                ->sum("npdf_nosabia.cantidad");
                            $total4 = NpdfNoSabia::select("npdf_nosabia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_nosabia.anio_id")
                                ->where("npdf_nosabia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_nosabia.mes_id", $trimestre4)
                                ->sum("npdf_nosabia.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NpdfNoSabia::select("npdf_nosabia.cantidad")
                                ->where("npdf_nosabia.distrito_id", $item->id)
                                ->where("npdf_nosabia.anio_id", $anio->id)
                                ->sum("npdf_nosabia.cantidad");
