...
use App\Models\Mes;
use App\Models\NpdfPorFamilia;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class NpdfPorFamiliaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/NpdfPorFamilias");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = NpdfPorFamilia::select("npdf_porfamilia.cantidad")
                    ->join("anios", "anios.id", "=", "npdf_porfamilia.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = NpdfPorFamilia::select("npdf_porfamilia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_porfamilia.anio_id")
                                ->where("npdf_porfamilia.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("npdf_porfamilia.cantidad");

                            $total2 = NpdfPorFamilia::select("npdf_porfamilia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_porfamilia.anio_id")
                                ->where("npdf_porfamilia.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = NpdfPorFamilia::select("npdf_porfamilia.cantidad")
                                    ...
                                    ->where("npdf_porfamilia.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("npdf_porfamilia.mes_id", $mes->id)
                                    ->sum("npdf_porfamilia.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = NpdfPorFamilia::select("npdf_porfamilia.cantidad")
                                ...
                                ->where("npdf_porfamilia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_porfamilia.mes_id", $trimestre1)
                                ->sum("npdf_porfamilia.cantidad");
                            $total2 = NpdfPorFamilia::select("npdf_porfamilia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_porfamilia.anio_id")
                                ->where("npdf_porfamilia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_porfamilia.mes_id", $trimestre2)
                                ->sum("npdf_porfamilia.cantidad");
                            $total3 = NpdfPorFamilia::select("npdf_porfamilia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_porfamilia.anio_id")
                                ->where("npdf_porfamilia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_porfamilia.mes_id", $trimestre3)
                                ->sum("npdf_porfamilia.cantidad");
                            $total4 = NpdfPorFamilia::select("npdf_porfamilia.cantidad")
                                ->join("anios", "anios.id", "=", "npdf_porfamilia.anio_id")
                                ->where("npdf_porfamilia.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("npdf_porfamilia.mes_id", $trimestre4)
                                ->sum("npdf_porfamilia.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = NpdfPorFamilia::select("npdf_porfamilia.cantidad")
                                ->where("npdf_porfamilia.distrito_id", $item->id)
                                ->where("npdf_porfamilia.anio_id", $anio->id)
                                ->sum("npdf_porfamilia.cantidad");
