...
use App\Models\Anio;
use App\Models\AvcsLugaresPublico;
use App\Models\Distrito;
use App\Models\Mes;
use Illuminate\Http\Request;
use Inertia\Inertia;

class AvcsLugaresPublicoController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/AvcsLugaresPublicos");
    }
        ...
        if ($filtro == 'todos') {
            ...
            $informacionArray = [
                [
                    "categories" => [],
                    "series" => [],
                    "titulo" => "Agresiones Verbales de Carácter Sexual en lugares Públicos",
            ...
            foreach ($array_info as $key_info => $info) {
                ...
                foreach ($distritos as $distrito) {
                    ...
                    $total = AvcsLugaresPublico::select("avcs_lugares_publicos.*")
                        ->join("anios", "anios.id", "=", "avcs_lugares_publicos.anio_id")
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($array_info as $key_info => $info) {
                            ...
                            foreach ($distritos as $distrito) {
                                ...
                                $total1 = AvcsLugaresPublico::select("avcs_lugares_publicos.*")
                                    ->join("anios", "anios.id", "=", "avcs_lugares_publicos.anio_id")
                                    ->where("anios.anio", $gestion1)
                                    ->where("distrito_id", $distrito->id)->sum($key_info);
                                $data1[] = (float)$total1;
                                $total2 = AvcsLugaresPublico::select("avcs_lugares_publicos.*")
                                    ->join("anios", "anios.id", "=", "avcs_lugares_publicos.anio_id")
                        ...
                        foreach ($meses as $key_registro => $mes) {
                            ...
                            foreach ($array_info as $key_info => $info) {
                                ...
                                foreach ($distritos as $distrito) {
                                    ...
                                    $total = AvcsLugaresPublico::select("avcs_lugares_publicos.*")
                                        ->join("anios", "anios.id", "=", "avcs_lugares_publicos.anio_id")
                                        ->where("avcs_lugares_publicos.mes_id", $mes->id)
                                        ->where("anios.anio", $gestion)
                                        ->where("distrito_id", $distrito->id)->sum($key_info);
                                    $data[] = (float)$total;
                                }
                        ...
                        foreach ($categories as $key_registro => $trim) {
                            ...
                            foreach ($array_info as $key_info => $info) {
                                ...
                                foreach ($distritos as $distrito) {
                                    ...
                                    $total = AvcsLugaresPublico::select("avcs_lugares_publicos.*")
                                        ->join("anios", "anios.id", "=", "avcs_lugares_publicos.anio_id")
                                        ->whereIn("avcs_lugares_publicos.mes_id", $trimestres[$key_registro])
                                        ->where("anios.anio", $gestion)
                                        ->where("distrito_id", $distrito->id)->sum($key_info);
                                    $data[] = (float)$total;
                                }
                    ...
                    foreach ($anios as $key_registro => $anio) {
                        ...
                        foreach ($array_info as $key_info => $info) {
                            ...
                            foreach ($distritos as $distrito) {
                                ...
                                $total = AvcsLugaresPublico::select("avcs_lugares_publicos.*")
                                    ->where("avcs_lugares_publicos.anio_id", $anio->id)
                                    ->where("distrito_id", $distrito->id)->sum($key_info);
                                $data[] = (float)$total;
                            }
                            $informacionArray[$key_registro]["series"][] = [
