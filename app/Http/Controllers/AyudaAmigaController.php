...
use App\Models\Mes;
use App\Models\AyudaAmiga;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class AyudaAmigaController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/AyudaAmigas");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = AyudaAmiga::select("ayuda_amiga.cantidad")
                    ->join("anios", "anios.id", "=", "ayuda_amiga.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = AyudaAmiga::select("ayuda_amiga.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_amiga.anio_id")
                                ->where("ayuda_amiga.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("ayuda_amiga.cantidad");

                            $total2 = AyudaAmiga::select("ayuda_amiga.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_amiga.anio_id")
                                ->where("ayuda_amiga.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = AyudaAmiga::select("ayuda_amiga.cantidad")
                                    ...
                                    ->where("ayuda_amiga.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("ayuda_amiga.mes_id", $mes->id)
                                    ->sum("ayuda_amiga.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = AyudaAmiga::select("ayuda_amiga.cantidad")
                                ...
                                ->where("ayuda_amiga.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_amiga.mes_id", $trimestre1)
                                ->sum("ayuda_amiga.cantidad");
                            $total2 = AyudaAmiga::select("ayuda_amiga.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_amiga.anio_id")
                                ->where("ayuda_amiga.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_amiga.mes_id", $trimestre2)
                                ->sum("ayuda_amiga.cantidad");
                            $total3 = AyudaAmiga::select("ayuda_amiga.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_amiga.anio_id")
                                ->where("ayuda_amiga.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_amiga.mes_id", $trimestre3)
                                ->sum("ayuda_amiga.cantidad");
                            $total4 = AyudaAmiga::select("ayuda_amiga.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_amiga.anio_id")
                                ->where("ayuda_amiga.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_amiga.mes_id", $trimestre4)
                                ->sum("ayuda_amiga.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = AyudaAmiga::select("ayuda_amiga.cantidad")
                                ->where("ayuda_amiga.distrito_id", $item->id)
                                ->where("ayuda_amiga.anio_id", $anio->id)
                                ->sum("ayuda_amiga.cantidad");
