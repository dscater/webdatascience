...
use App\Models\Mes;
use App\Models\AyudaFelcv;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Inertia\Inertia;

class AyudaFelcvController extends Controller
{
    public function index()
    {
        return Inertia::render("AnalisisDatos/AyudaFelcvs");
    }
    ...
    public function grafico(Request $request)
    {
        ...
        if ($filtro == 'todos') {
            ...
            foreach ($distritos as $distrito) {
                $total = AyudaFelcv::select("ayuda_felcv.cantidad")
                    ->join("anios", "anios.id", "=", "ayuda_felcv.anio_id")
                ...
                case 'comparargestion':
                    ...
                    if ($gestion1 && $gestion2 && $gestion1 > 0 && $gestion2 > 0) {
                        ...
                        foreach ($distritos as $item) {
                            $total1 = AyudaFelcv::select("ayuda_felcv.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_felcv.anio_id")
                                ->where("ayuda_felcv.distrito_id", $item->id)
                                ->where("anios.anio", $categories[0])
                                ->sum("ayuda_felcv.cantidad");

                            $total2 = AyudaFelcv::select("ayuda_felcv.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_felcv.anio_id")
                                ->where("ayuda_felcv.distrito_id", $item->id)
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            foreach ($meses as $key_mes => $mes) {
                                ...
                                $total = AyudaFelcv::select("ayuda_felcv.cantidad")
                                    ...
                                    ->where("ayuda_felcv.distrito_id", $item->id)
                                    ->where("anios.anio", $gestion)
                                    ->where("ayuda_felcv.mes_id", $mes->id)
                                    ->sum("ayuda_felcv.cantidad");
                    ...
                    if ($gestion > 0) {
                        ...
                        foreach ($distritos as $distritos_index => $item) {
                            ...
                            $total1 = AyudaFelcv::select("ayuda_felcv.cantidad")
                                ...
                                ->where("ayuda_felcv.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_felcv.mes_id", $trimestre1)
                                ->sum("ayuda_felcv.cantidad");
                            $total2 = AyudaFelcv::select("ayuda_felcv.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_felcv.anio_id")
                                ->where("ayuda_felcv.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_felcv.mes_id", $trimestre2)
                                ->sum("ayuda_felcv.cantidad");
                            $total3 = AyudaFelcv::select("ayuda_felcv.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_felcv.anio_id")
                                ->where("ayuda_felcv.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_felcv.mes_id", $trimestre3)
                                ->sum("ayuda_felcv.cantidad");
                            $total4 = AyudaFelcv::select("ayuda_felcv.cantidad")
                                ->join("anios", "anios.id", "=", "ayuda_felcv.anio_id")
                                ->where("ayuda_felcv.distrito_id", $item->id)
                                ->where("anios.anio", $gestion)
                                ->whereIn("ayuda_felcv.mes_id", $trimestre4)
                                ->sum("ayuda_felcv.cantidad");
                ...
                case 'anual':
                    ...
                    foreach ($distritos as $distritos_index => $item) {
                        ...
                        foreach ($anios as $key_anio => $anio) {
                            ...
                            $total = AyudaFelcv::select("ayuda_felcv.cantidad")
                                ->where("ayuda_felcv.distrito_id", $item->id)
                                ->where("ayuda_felcv.anio_id", $anio->id)
                                ->sum("ayuda_felcv.cantidad");
